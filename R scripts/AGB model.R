...
#here we check to see which random effect is the best to use
...
#slope with age for each study along with whether equations used
#in allometry were local or more general

#now we need to check whether it's best to include log or linear terms
M1lin<-lmer(Change~Age+Disturbance*Age+Type+(Age|Ran)+(1|LG),data=AGB2,REML=F)
M1log<-lmer(Change~log(Age)+Disturbance*Age+Type+(Age|Ran)+(1|LG),data=AGB2,REML=F)
AIC(M1lin,M1log)

#the log model looks best
#run it again but with REML=T
M1<-lmer(Change~log(Age)+Disturbance*log(Age)+Type+(Age|Ran)+(1|LG),data=AGB2,REML=T)
...
#plot these results to check they are sensible
